...
def mcp_sse():
    ...
    def event_stream():
        ...
        # Keep alive for up to 5 minutes to prevent worker thread starvation on sync WSGI
        for _ in range(10):
            time.sleep(30)
            yield ": keep-alive\n\n"
    ...
    try:
        if method == "initialize":
            ...
            for tool in mcp_server._tool_manager.list_tools():
                input_schema = tool.parameters if isinstance(tool.parameters, dict) else tool.parameters.model_json_schema()
                tools_list.append({
                    "name": tool.name,
            ...
            tool = next((t for t in mcp_server._tool_manager.list_tools() if t.name == tool_name), None)
            if not tool:
